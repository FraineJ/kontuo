...
class AddDebtPaymentDialog extends StatefulWidget {
  ...
  static Future<bool?> show(BuildContext context, Debt debt, UserProfile? userProfile) {
    return showModalBottomSheet<bool>(
      context: context,
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
      builder: (context) => AddDebtPaymentDialog(debt: debt, userProfile: userProfile),
    );
  }
...
class _AddDebtPaymentDialogState extends State<AddDebtPaymentDialog> {
  ...
  double get _remainingAmount => widget.debt.remainingAmount;
  ...
  Future<void> _savePayment() async {
    ...
    final newRemainingAmount = widget.debt.remainingAmount - amount;
    final finalRemainingAmount =
    newRemainingAmount < 0 ? 0.0 : newRemainingAmount.toDouble();


    await _storageService.updateDebt(
      widget.debt.copyWith(remainingAmount: finalRemainingAmount),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final progress = widget.debt.progress;
    final bank = ColombianBanks.getBankByName(widget.debt.name);
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  Text(
                                    currencyFormat.format(widget.debt.totalAmount),
                                    style: const TextStyle(
                                      fontSize: 18,
                                      fontWeight: FontWeight.w600,
                                      color: AppTheme.textPrimary,
